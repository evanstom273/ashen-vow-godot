...
| Value | Amount |
| --- | --- |
| Player health / stamina / focus | 650 / 120 / 100 |
...
| Spell template magic damage / focus cost | 180 / 18 |
...
Default growth is +30 health per Vigour point, +3 stamina and +1.5 load capacity per Endurance point, and +3 focus per Intelligence point. Offensive attributes use the existing editable scaling coefficients. These are custom Souls-inspired values, not an exact recreation of another game's formulas. The values have not been playtested.

## What to edit

- **Attributes:** Vigour, Endurance, Strength, Dexterity, Intelligence, Faith and Arcane. All are stored in `AttributeStats` resources.
- **Vitals:** base health, stamina, focus, equipment-load capacity, attribute growth, stamina recovery, poise, poise recovery, stagger duration and damage invulnerability.
- **Weapons:** identity/category, weight, attribute requirements, light attack and blade colour.
- **Attacks:** physical/magic/fire/lightning/holy damage, attribute scaling, stamina/focus costs, poise damage, knockback, windup/active/recovery timing, reach, hit radius, arc, movement during attacks, lunge, active-frame interruption protection, hit-stop and feedback.
...
- Stamina and load capacity use Endurance; focus uses Intelligence. Each has its own editable growth coefficient. There is no separate Mind stat.
...
- Current HP, stamina, focus, poise and copied attributes live on each actor. Damaging one enemy never mutates a shared `.tres`.
...
Spell casting/projectile spawning, boss phase orchestration/fog gates, status buildup, heavy attacks, skills, guarding, critical attacks, rewards, equipment-load penalties and inventory/equipment switching are not implemented. Their fields are explicitly labelled data-only where applicable. Weight and load capacity are available for future equipment logic; they do not yet change roll speed. Focus is tracked, spent by attack definitions and restored by the shrine, but has no casting input or dedicated HUD yet. Custom equipped-scene replacement is also data-only; the existing polygon weapon uses the configured blade colour.
...
No tests or Godot runs were performed for this resource migration, as requested.
